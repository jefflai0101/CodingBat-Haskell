...
groupSumClump :: [Int] -> Int -> Int -> Int -> Bool
groupSumClump (x:xs) target sum previous
      | x == previous                = groupSumClump xs target (sum+x) x
      | otherwise                    = groupSumClump xs target (sum+x) x || groupSumClump xs target sum previous
groupSumClump [] target sum previous = target == sum
